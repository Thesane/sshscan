...
// 		    printf("th %lu exited due to connect failed\n",(*iter));
		    break;
		  }
	...
	for(iter=parg->thread_list->begin();iter!= parg->thread_list->end();++iter)
	{
	  if((*iter) == pthread_self())
	    {
	      parg->thread_list->remove((*iter));
// 	      printf("th %lu exited due to complete flag\n",(*iter));
	      break;
	    }
...
//		parg->ip, parg->port ,parg->user , parg->password);
	...
	for(iter=parg->thread_list->begin();iter!= parg->thread_list->end();++iter)
	{
	  if((*iter) == pthread_self())
	    {
	      parg->thread_list->remove((*iter));
// 	      printf("th %lu exited due to socket failed\n",(*iter));
	      break;
	    }
...
// 	sleep(rand() % 10 + 1);
	...
	for(iter=parg->thread_list->begin();iter!= parg->thread_list->end();++iter)
	{
	  if((*iter) == pthread_self())
	    {
	      parg->thread_list->remove((*iter));
// 	      printf("th %lu exited due to password auth not avail\n",(*iter));
	      break;
	    }
	...
	if ((fp = fopen(parg->setting->path_log, "a")) == NULL)
	{
		...
		if((*iter) == pthread_self())
		  {
		    parg->thread_list->remove((*iter));
// 		    printf("th %lu exited  due to log file \n",(*iter));
		    break;
		  }
	...
	parg->complete = 1;
		...
		if((*iter) == pthread_self())
		  {
		    parg->thread_list->remove((*iter));
// 		    printf("th %lu exited  due to success\n",(*iter));
		    break;
		  }
	...
	parg->complete = 1;
		...
		if((*iter) == pthread_self())
		  {
		    parg->thread_list->remove((*iter));
// 		    printf("th %lu exited due to auth failed\n",(*iter));
		    break;
		  }
